...
def load_BCI2a_data(data_path, subject, training, all_trials = True):
    ...
    t1 = int(1.5*fs)  # start time_point1.5
    t2 = int(6*fs)    # end time_point6
...
def get_data(path, subject, dataset = 'BCI2a', classes_labels = 'all', LOSO = False, isStandard = True, isShuffle = True):
    ...
    if LOSO:
        ...
        if (dataset == 'BCI2a'):
            ...
            sio.savemat('F:/0909dataset/datasetsprocess/bci2a/Dataset_%d.mat' % int(subject+1), {'Dataset':X_train})
            sio.savemat('F:/0909dataset/datasetsprocess/bci2a/Labels_%d.mat' % int(subject+1), {'Labels': y_train})
        elif (dataset == 'CS2R'):
