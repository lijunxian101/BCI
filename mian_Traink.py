...
def train(dataset_conf, train_conf, results_path):
    ...
    #加载数据
    DIR = 'data/ph/'
    data, labels = Datasetload(DIR=DIR)
...
#k折验证
    ...
    for train_index, test_index in kf.split(data):
        ...
        i = 1
        print('k is', i)
        X_train, y_train= data[train_index], data[test_index]
        train_labels, test_labels = labels[train_index], labels[test_index]

        X_train = X_train.reshape(X_train.shape[0], 1, X_train.shape[1], X_train.shape[2])
        y_train_onehot = to_categorical(train_labels)

        X_val = y_train.reshape(y_train.shape[0], 1, y_train.shape[1], y_train.shape[2])
        y_val_onehot = to_categorical(test_labels)
        ...
        filepath = './data/subject-{}.h5'.format(1)
...
def test(model, dataset_conf, results_path, allRuns=True):
    ...
    for sub in range(n_sub):  # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for seed in range(len(runs)):
            ...
            accy=accuracy_score(labels, y_pred)
            print("accy",accy)
...
def run():
    ...
    train_conf = {'batch_size': 10, 'epochs': 500, 'patience': 100, 'lr': 0.001, 'n_train': 1,
